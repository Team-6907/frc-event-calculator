# FRC 赛事积分分析器

[English](README.md) | [中文（简体）](README.zh-CN.md)

用于分析 FRC（FIRST 机器人竞赛）赛事并计算 2025+ 和 2026 赛季的区域积分。支持增强资格跟踪的整季区域池计算，提供命令行与现代化 Streamlit 仪表板，内置本地缓存与实时进度显示。

## 功能特性

- 赛事分析：队伍、排名、联盟、比赛、奖项
- **2025+ 区域积分**：排位、联盟选择、淘汰晋级、奖项、新秀加分
- **2026 区域晋级**：美国站（3 席位）vs 国际站（4 席位），增强资格跟踪
- **区域池**：每周自动晋级与席位分配（2025）；每站前 3（2026，可回填）
- **Streamlit 仪表板**：现代化网页界面，包含赛事统计、雷达图和区域池可视化
- **单站积分跟踪**：Event1/Event2 积分细分，用于详细性能分析
- **缓存**：自动将 FRC Events API 响应以 JSON 缓存到 `cache/`
- **进度指示器**：CLI 和网页界面在长时间抓取时显示进度与剩余时间
- **Statbotics EPA**：可选的分站期望得分（EPA）查询与综合分析
- **命令行与编程接口**：表格与 JSON 输出

## 环境要求

- Python 3.10+
- FRC Events API 访问凭据（`AUTH_USERNAME`, `AUTH_TOKEN`）
- 首次运行需要网络（之后多为本地缓存）

## 安装

```bash
git clone <repository-url>
cd frc-event-calculator
pip install -e .
```

## 配置

在项目根目录创建 `.env` 文件：

```
AUTH_USERNAME=your_username
AUTH_TOKEN=your_api_token
```

API 凭据申请：官方 FRC Events API 文档 https://frc-api.firstinspires.org/

## 命令行用法

首次运行会较慢（缓存预热），命令行会显示实时进度与剩余时间。

```bash
# 分析单个赛事
frc-calculator analyze-event 2024 AZVA

# 计算队伍的区域积分
frc-calculator calculate-points 2024 AZVA 1234 --verbose
frc-calculator calculate-points 2024 AZVA 1234 --json

# 查看区域池榜单（2025 规则）
frc-calculator regional-pool 2025 --week 6 --top 50

# 查看区域池榜单（2026 规则，增强跟踪）
frc-calculator regional-pool 2026 --week 3 --use-season 2026
```

说明：
- `regional-pool` 会先计算本赛季赛事数量（较快），随后构建每个赛事（较慢，显示进度条）。数据会缓存到 `cache/`，再次运行更快。

## Streamlit 可视化面板（v2.7.0）

现代化、用户友好的 Streamlit 面板，全新 UI/UX 设计，提供全面的 FRC 赛事分析工具。

安装：

```bash
pip install -e .
pip install -r requirements.txt  # 确保安装 streamlit
```

运行：

```bash
streamlit run src/frc_calculator/ui/streamlit_app.py
```

### 主要功能：
- **🔐 增强凭据设置**：从侧边栏移至主界面，支持内联验证、清晰错误提示和更好的安全指示器
- **🚀 自动获取赛事列表**：当提供凭据时，应用启动时自动获取2023、2024和2025年赛事列表，无需手动加载赛事
- **🧭 全局上下文栏（新增）**：在“事件作用域”标签页提供赛季/赛事统一选择，在“赛季作用域”的区域池标签页提供赛季/规则年份/周次统一选择；选择在标签页之间保持一致，并同步到 URL 便于分享
- **🏆 赛事分析**：智能赛事选择（下拉菜单 + 手动覆盖）、改进的进度追踪、格式化数据表格与队伍信息
- **📊 积分计算器**：队伍选择改为来自所选赛事的下拉列表（若无数据则回退为手动输入）；结果包含可视化积分细分与队伍详情
- **🏁 区域池**：赛季/规则年份/周次移动到全局上下文栏，配合详细进度追踪、**2026 单站积分**（Event1/Event2 列），资格状态指示器和摘要统计信息
- **📈 赛事统计**：全面的赛事分析，包含比分趋势、淘汰赛表现、EPA 集成和统计洞察
- **📡 赛事雷达图**：新增！8维雷达图分析，提供全面的赛事洞察，涵盖竞争力、队伍实力和淘汰赛表现等多个性能指标

### UI 改进：
- 采用表情符号图标的现代设计，更好的视觉层级结构
- 移动友好的表单，使用文本输入替代数字调节器
- 增强错误处理，可展开详情和上下文帮助
- 更好的数据可视化，包含列配置和状态指示器
- 通过查询参数实现深链与状态恢复：`tab`、`scope`、`season`、`event`、`team`、`pool_week`、`pool_rules`

## 编程接口

```python
from frc_calculator.models.event import Event
from frc_calculator.services.season import Season

# 单一赛事
event = Event(2024, "AZVA")
team = event.get_team_from_number(1234)
points = team.regional_points_2025()
print(points)

# 区域池
season = Season(2025, useSeason=2025)
pool_w6 = season.regional_pool_2025(weekNumber=6)
print(list(pool_w6.items())[:5])
```

## 缓存与数据

- 缓存文件保存在 `cache/`（自动创建），格式与 FRC Events API 响应一致。
- 再次运行将优先使用缓存，如缺字段再按需请求。
- 若需强制刷新，可删除 `cache/` 目录。

## 项目结构

```
src/frc_calculator/
  cli/app.py                  # CLI commands
  config/constants.py         # Season constants (2025/2026)
  data/frc_events.py          # FRC Events API + caching
  data/statbotics.py          # Statbotics EPA client
  models/{event,team,alliance,match}.py
  services/season.py          # Season builder + regional pool
  utils/{io_utils,math_utils}.py
  ui/components.py            # 共享 Streamlit 组件与工具（选择器、进度、校验）
  ui/charts.py                # 雷达图渲染工具
  ui/streamlit_app.py         # 主界面（各标签页使用共享组件）
```

## 开发

```bash
pip install -e .
pip install -r requirements.txt

# Run tests (note: uses live network unless you provide cached data)
python Test.py
```

提示：
- CLI 会显示进度；编程接口默认静默，如需可传入进度回调。
- 当前测试依赖在线 API（或本地缓存）。如需可重复测试，建议先将响应缓存到 `cache/`。

## 许可

Add your license here.

## 支持

- 在仓库中提交 Issue（请附带复现步骤与日志）
- 首次运行需确认 `.env` 配置正确且网络可用

## 版本记录

### v2.6.2（雷达图主题修复）
- **🐛 热修复**：修复亮色主题下雷达图文字不可见问题，使用 `st.context.theme.type` 实现正确的主题检测
- **🎨 主题兼容性**：雷达图现在使用适当的文本颜色（暗色主题用白色，亮色主题用黑色）和网格颜色
- **🔄 注意**：由于 Streamlit 主题检测限制，切换主题后用户需手动刷新页面

### v2.6.1（设置弹窗 + 状态栏）
- **⚙️ 设置弹窗**：将凭据与缓存管理移动到弹窗，减少主界面占用，遵循“按需展示”最佳实践
- **🧭 精简状态栏**：显示 API 状态（在线/仅缓存）与当前赛季/赛事，并提供统一的设置入口
- **✅ 校验体验**：单一“Validate”按钮（避免换行），静默刷新赛事列表，统一成功提示
- **🖼️ 布局优化**：放宽弹窗宽度，避免表单拥挤；微调文案

### v2.6.0（作用域上下文栏 + 队伍下拉）
- **🧭 作用域上下文栏**：统一赛季/赛事选择，支持事件作用域与赛季作用域，并同步到 URL 以便分享
- **📊 积分页队伍下拉**：队伍号由所选赛事动态生成的下拉列表提供（无数据时回退为手动输入）
- **🏁 区域池集成**：赛季、规则年份与周次集中到上下文栏（Top N 仍在标签页内）
- **🔗 URL 同步**：选择会镜像到查询参数，支持分享链接与状态恢复
- **🧹 重构**：移除“赛事分析/积分/赛事统计”页的重复选择器，统一使用共享上下文

### v2.5.0（UI 组件重构）
- **🧩 共享 UI 组件**：将通用的 Streamlit UI 模式抽取到 `ui/components.py`（赛事选择、数值校验、进度回调）
- **📈 图表模块**：将雷达图渲染迁移至 `ui/charts.py`，结构更清晰、样式更易维护
- **🧹 减少重复**：重构“赛事分析、积分计算、赛事统计、赛事雷达图”标签页以复用共享组件，功能无变化
- **🔧 小幅清理**：移除未使用的导入，统一数值校验逻辑；保持原有交互体验

### v2.4.3（自动获取赛事列表）
- **🚀 新增自动获取功能**：当提供凭据时，应用启动时自动获取2023、2024和2025年赛事列表
- **⚡ 改进启动体验**：通过自动预加载赛事数据，消除"未加载赛事"消息
- **🔄 智能缓存管理**：赛事列表加载后的智能缓存失效和UI刷新
- **📱 更好的用户体验**：输入凭据后无需手动重启应用
- **🔧 会话状态管理**：增强的状态管理，实现无缝的赛事数据加载和UI更新

### v2.4.2（进度追踪错误修复）
- **🐛 错误修复**：修复了区域池进度追踪差异（之前显示"20/7 赛事"，现在正确显示"7/7 赛事"）
- **📊 准确赛事计数**：进度条现在只计算请求周次的赛事，而不是所有周次
- **⚡ 性能改进**：赛季构建现在只处理到请求周次为止的赛事
- **📡 新增赛事雷达图标签页**：8维雷达图分析，支持交互式 Plotly 可视化
- **🎨 色盲友好设计**：高对比度、色盲安全的调色板，提升可访问性
- **📊 雷达图维度**：整体竞争力、排名积分难度、非季后赛队伍实力（TANK）、回归队伍实力（HOME）、老牌队伍数量（REIGN）、季后赛竞争力（Title）和决赛表现（CHAMP）
- **🔍 增强分析**：全面的赛事分析，包含维度细分和等级分类
- **📈 交互式可视化**：基于 Plotly 的雷达图，提供详细解释指南和指标细分
- **⚡ 性能优化**：简化的 EPA 进度报告，提升用户体验
- **🔄 增强 EPA 处理**：改进的批处理，包含回退机制以提高可靠性

### v2.2.0（赛事统计与分析）
- **📈 新增赛事统计标签页**：全面的赛事分析，包含比分趋势、淘汰赛表现、EPA 集成和统计洞察
- **🤖 EPA 集成**：无缝 Statbotics EPA 数据获取，支持进度追踪和缓存
- **📊 高级分析**：排位赛与淘汰赛比分对比、比赛得分分布和队伍表现指标
- **🔧 增强缓存**：改进的缓存管理，独立的 EPA 缓存控制和更好的用户反馈
- **⚡ 性能优化**：长时间操作的更好进度指示器和更智能的数据加载

### v2.1.0（UI/UX 全面改版）
- **🎨 完整 UI 重新设计**：现代化界面，表情符号图标，更好的视觉层级和间距
- **📱 移动友好表单**：使用文本输入替代数字调节器，提供更好的移动端体验
- **🔐 增强凭据管理**：从侧边栏移至主界面，支持内联验证和更清晰的错误消息
- **📊 更好的数据可视化**：增强表格格式、列配置、进度指示器和摘要统计信息
- **🚀 改进用户体验**：上下文帮助、可展开错误详情和可操作的指导消息

### v2.0.0（重构）
- 新增 Streamlit 仪表盘（下拉选择赛事 + 手动覆盖）
- 支持凭据校验，错误更清晰；API 客户端更健壮并带超时
- 区域池构建进度反馈增强（状态 + 最近赛事列表）
- 更新文档（README、README.zh-CN、CLAUDE.md）

### v1.0.0
- 重构为模块化包，提供 CLI 与实时进度
- 保持原有计算逻辑，并迁移到 `src/frc_calculator`
- 自动创建本地缓存目录，体验更佳
